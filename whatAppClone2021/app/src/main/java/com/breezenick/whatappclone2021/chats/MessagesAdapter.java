...
public  class MessagesAdapter extends RecyclerView.Adapter<MessagesAdapter.MessageViewHolder> {
    ...
    public MessageViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.message_layout,
                parent , false);
        ...
        public ActionMode.Callback actionModeCallBack = new ActionMode.Callback() {
            ...
            public boolean onActionItemClicked(ActionMode actionMode, MenuItem menuItem) {
                String selectedMessageId =
                        String.valueOf(selectedView.getTag(R.id.TAG_MESSAGE_ID));
                String selectedMessage =
                        String.valueOf(selectedView.getTag(R.id.TAG_MESSAGE));
                String selectedMessageType =
                        String.valueOf(selectedView.getTag(R.id.TAG_MESSAGE_TYPE));
                ...
                switch (itemId){
                    case R.id.mnuDelete:
                        //Toast.makeText(context , "Delete Option Checked", Toast.LENGTH_SHORT).show();
                    ...
                    case R.id.mnuDownload:
                        //Toast.makeText(context , "Download Option Checked", Toast.LENGTH_SHORT).show();
                        if(context instanceof  ChatActivity)
                        {
                        ...
                        break;
                    case R.id.mnuShare:
                        //Toast.makeText(context , "Share Option Checked", Toast.LENGTH_SHORT).show();
                        if(selectedMessageType.equals(Constants.MESSAGE_TYPE_TEXT)){
                            ...
                            if(context instanceof  ChatActivity)
                            {
                                ((ChatActivity)context).downloadFile(selectedMessageId,
                                        selectedMessageType , true);
                            }
                        }
                        actionMode.finish();
                        break;
                    case R.id.mnuForward:
                        if(context instanceof  ChatActivity){
                            ((ChatActivity)context).forwardMessage(selectedMessageId,
                                    selectedMessage, selectedMessageType);

                        }
                        //context.startActivity(new Intent(context, SelectFriendActivity.class));
                        //Toast.makeText(context , "Forward Option Checked", Toast.LENGTH_SHORT).show();
                        actionMode.finish();
                        break;
